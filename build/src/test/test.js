...
describe('tokenizer function', () => {
    it('should tokenize a simple arithmetic expression', () => {
        const input = '3 + 5';
        const expected = [3, '+', 5];
        console.log((0, index_1.tokenizer)(input));
        expect((0, index_1.tokenizer)(input)).toEqual(expected);
    });
    ...
    it('should handle invalid characters gracefully', () => {
        const input = '7a + 2';
        const expected = [7, '+', 2]; // Assuming non-numeric characters are ignored
        console.log((0, index_1.tokenizer)(input));
        expect((0, index_1.tokenizer)(input)).toEqual(expected);
    });
});
describe('infixToPostfix 함수 테스트', () => {
    it('숫자만 있는 경우, 후위 표기법으로 변환해야 한다', () => {
        const input = [3, 4, 5];
        const expected = [3, 4, 5]; // 숫자만 있을 경우 그대로 출력
        console.log((0, index_1.infixToPostfix)(input));
        expect((0, index_1.infixToPostfix)(input)).toEqual(expected);
    });
    it('단일 연산자가 있는 경우, 후위 표기법으로 변환해야 한다', () => {
        const input = [3, '+', 5];
        const expected = [3, 5, '+']; // 연산자가 뒤로 가야 한다.
        console.log((0, index_1.infixToPostfix)(input));
        expect((0, index_1.infixToPostfix)(input)).toEqual(expected);
    });
    it('두 개의 연산자가 있을 때, 연산자 우선순위에 따라 후위 표기법으로 변환해야 한다', () => {
        const input = [3, '+', 5, '*', 2];
        const expected = [3, 5, 2, '*', '+']; // '*' 연산자가 '+' 보다 우선순위가 높다.
        console.log((0, index_1.infixToPostfix)(input));
        expect((0, index_1.infixToPostfix)(input)).toEqual(expected);
    });
    it('복잡한 연산식에서 후위 표기법으로 정확히 변환해야 한다', () => {
        const input = [3, '+', 5, '*', 2, '-', 8];
        const expected = [3, 5, 2, '*', '+', 8, '-']; // 연산자 우선순위 고려
        console.log((0, index_1.infixToPostfix)(input));
        expect((0, index_1.infixToPostfix)(input)).toEqual(expected);
    });
});
describe('calculatePostfix 함수 테스트', () => {
    it('단일 연산이 있는 경우, 결과를 정확히 계산해야 한다', () => {
        const input = [3, 5, '+'];
        const expected = 8;
        expect((0, index_1.calculatePostfix)(input)).toBe(expected);
    });
    ...
    it('잘못된 후위 표기식이 입력될 경우, 에러를 던져야 한다', () => {
        const input = [3, '+']; // 연산에 필요한 피연산자가 부족함
        expect(() => (0, index_1.calculatePostfix)(input)).toThrow('잘못된 후위표기식 입니다.');
    });
});
